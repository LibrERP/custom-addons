...
class AccountBankStatementImportPayPalParser(models.TransientModel):
    ...
    def _parse_lines(self, mapping, data_file, currency_code):
        ...
        header = [column.strip() for column in next(csv_data)]
        ...
        description_column = get_index_or_none(header, mapping.description_column)
        type_column = get_index_or_none(header, mapping.type_column)
        from_email_address_column = get_index_or_none(
            header, mapping.from_email_address_column
        )
        to_email_address_column = get_index_or_none(
            header, mapping.to_email_address_column
        )
        invoice_id_column = get_index_or_none(header, mapping.invoice_id_column)
        subject_column = get_index_or_none(header, mapping.subject_column)
        note_column = get_index_or_none(header, mapping.note_column)
        bank_name_column = get_index_or_none(header, mapping.bank_name_column)
        bank_account_column = get_index_or_none(header, mapping.bank_account_column)
