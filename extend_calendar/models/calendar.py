...
class Hours(models.Model):
    ...
    def _compute_duration(self):
        """
            Computes duration time, starting, ending values on daily base.
        """
        for interval in self:
...
class Days(models.Model):
    ...
    code = fields.Char(
        ...
        required=True,
        translate=True, 
        help='The short name of the day.'
    )
    day_number = fields.Integer(
        string='Day Number',
        help='The day number, in the week. \nYou can use this field for quick search.'
    )
    interval_ids = fields.Many2many(
        'res.calendar.intervals', 
        string='Time Intervals'
    )


class Weeks(models.Model):
    _name = "res.calendar.week"
    _description = "Calendar Weeks"

    name = fields.Char(
        string='Week Name', required=True, translate=True, help='The full name of the week.')
    week_code = fields.Char(
        string='Week Code',
        help='The week code in two chars. \nYou can use this field for quick search.')
    day_ids = fields.Many2many('res.calendar.day', string='Days of this week')
...
class Calendar(models.Model):
    ...
    @api.model
    def getcalendar(self, partner_id, year=None):
        """
            Gets work calendar for partner country (and year or current)
        """
        ret = None
        if partner_id and partner_id.country_id and partner_id.country_id.code:
            iso_country = "{}".format(partner_id.country_id.code).upper()
            this_year = WorkCalendar(iso_country=iso_country, year=year)
            if this_year:
                ret = this_year.calendar
        return ret
